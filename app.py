...
r = requests.get('https: // pro-api.coinmarketcap.com/v1/cryptocurrency /
                 listings/latest?limit=200 & CMC_PRO_API_KEY=' +
...
r = requests.get('https: // pro-api.coinmarketcap.com//v1/global-metrics /
                 quotes/latest? & CMC_PRO_API_KEY=' +
                 os.environ.get('API_KEY'))
results = r.json()
    ...
    if request.method == "POST":
        ...
        if exsisting_user is None:
            ...
            user.insert_one({'name': request.form['username'],
                            'password': hashpassword, 'positions': []})
            session['username'] = request.form['username']
            return redirect(url_for('index'))
        return render_template('error_page.html')
    ...
    if 'username' in session:
        user = mongo.db.user.update_one({'name': session['username']},
                                        {'$push': {'positions': {
                                         ...
                                         'amount': float(request.form.get
                                                         ('amount')),
                                         'price_paid': float(request.form.get
                                                             ('price_paid')),
                                         'date_of_purchase': request.form.get
                                                        ('date_of_purchase'),
...
def get_my_bagz(username):
    if 'username' in session:
        ...
        return render_template('my_crypto.html', positions=user['positions'],
...
def edit_bag(username, bag_id):
    if 'username' in session:
        ...
        for pos in positions:
            if pos['_id'] == bag_id:
                bag = pos
        return render_template("edit_bag.html", data=data,
                               username=session['username'],
                               bag_id=bag_id, bag=bag)
    return render_template('error_page.html')
...
def update_bag(username, bag_id):
    user = mongo.db.user.update_one({'name': session['username'],
                                    'positions._id': bag_id},
                                    {'$set': {'positions.$.asset':
                                              request.form.get('asset'),
                                              'positions.$.amount':
                                              float(request.form.get
                                                    ('amount')),
                                              'positions.$.price_paid':
                                              float(request.form.get
                                                    ('price_paid')),
                                              'positions.$.date_of_purchase':
                                              request.form.get
                                              ('date_of_purchase'),
                                              'positions.$.favorite':
